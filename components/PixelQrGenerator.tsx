...
import { generateQR } from "@/lib/pyodide/loader";
...
export function PixelQrGenerator() {
  ...
  const scale = 8; // Reduced scale for compatibility
  ...
  const convertHeicToPng = async (file: File): Promise<File> => {
    try {
      ...
      throw new Error(
        "Could not convert HEIC image. Please try using a different image format.",
      );
    }
  };
  ...
  const generateQrCode = async () => {
    ...
    if (!isSupported) {
      showToast("WebAssembly is not supported in this browser.", "error");
      return;
    }
    if (pyodideError) {
      showToast(`WASM error: ${pyodideError.message}`, "error");
      return;
    }
    if (!pyodideReady || !pyodide) {
      showToast("Loading engine... Try again in a moment.", "error");
      return;
    }
    ...
    try {
      const data = await generateQR(pyodide, {
        url: qrContent,
        scale,
        kind: "png",
        background: imageBytes || undefined,
      });
      ...
      showToast(`Error generating QR: ${errorMessage}`, "error");
                    ...
                    <div className="flex-1 flex items-center justify-center relative group">
                      <div className="p-2 md:p-4 bg-white/5 rounded-lg border border-gray-700/30 w-full h-full flex items-center justify-center overflow-hidden">
                        <img
                          ...
                          alt="Generated"
                          className={cn(
                            "max-w-full max-h-full object-contain rounded transition-opacity duration-500",
